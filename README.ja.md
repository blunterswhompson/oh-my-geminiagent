...
|   🔍   | **`/init-deep`**                                         | プロジェクト全体にわたって階層的な `GEMINI.md` ファイルを自動生成します。トークン効率とエージェントのパフォーマンスの両方を向上させます。                                                                                      |
...
`/init-deep` を実行してください。階層的な `GEMINI.md` ファイルを生成します：

```
project/
├── GEMINI.md              ← プロジェクト全体のコンテキスト
├── src/
│   ├── GEMINI.md          ← src 専用のコンテキスト
│   └── components/
│       └── GEMINI.md      ← コンポーネント専用のコンテキスト
```
