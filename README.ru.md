...
| 🔍   | **`/init-deep`**                                         | Автоматически генерирует иерархические файлы `GEMINI.md` по всему проекту. Отлично работает на эффективность токенов и производительность агента.                                                                                |
...
Запустите `/init-deep`. Будут сгенерированы иерархические файлы `GEMINI.md`:

```
project/
├── GEMINI.md              ← контекст всего проекта
├── src/
│   ├── GEMINI.md          ← контекст для src
│   └── components/
│       └── GEMINI.md      ← контекст для компонентов
```
...
- **Инъекция контекста**: Автоматическое добавление GEMINI.md, README.md, условных правил
